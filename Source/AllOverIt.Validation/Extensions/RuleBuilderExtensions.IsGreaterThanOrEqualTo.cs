...
namespace AllOverIt.Validation.Extensions
{
    ...
    {
        /// <summary>Defines a validator on the current rule builder that will fail if the value of the property is greater
        /// than or equal to a specified value. The error code is set to <see cref="ValidationErrorCode.OutOfRange"/>.</summary>
        ...
        {
            return ruleBuilder
                .SetValidator(new GreaterThanOrEqualToContextValidator<TType, TProperty, TContext>(valueResolver))
                .WithErrorCode(ValidationErrorCode.OutOfRange);
        }
