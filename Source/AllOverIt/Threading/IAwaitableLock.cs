...
namespace AllOverIt.Threading
{
    ...
    public interface IAwaitableLock : IDisposable
    {
        ...
        Task<bool> TryLockAsync(int milliseconds, CancellationToken cancellationToken);
        ...
        /// <returns>A task that will complete with a result of true if the current thread successfully
        ///  obtained the lock, otherwise with a result of false.</returns>
        Task<bool> TryLockAsync(TimeSpan timeSpan, CancellationToken cancellationToken);

        /// <summary>Releases the lock held by the current thread. This must only be called if the thread
        /// successfully obtained the lock.</summary>
        void Release();
    }
}